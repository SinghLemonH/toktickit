...
function renderPage() {
  sessionStorage.setItem("toktickit.selectedRequester", JSON.stringify({ id: 1, name: "Jennifer Anderson" }));
  return render(<MemoryRouter><RequesterProvider><CreateTicket /></RequesterProvider></MemoryRouter>);
}
...
describe("CreateTicket", () => {
  beforeEach(() => {
    ...
    vi.spyOn(api, "getActiveRelatedSystems").mockResolvedValue([{ id: 7, name: "Corporate Laptop" }]);
  });
  ...
  it("disables the Submit button while a request is in flight (UI-04)", async () => {
    ...
    await userEvent.type(screen.getByLabelText(/Description/i), "This description is long enough to satisfy the minimum character requirement.");
